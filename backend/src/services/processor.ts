  ...
  try {
    ...
    for (const lesson of transcribedLessons) {
      try {
        ...
        db.prepare(
          'INSERT INTO summaries (lesson_id, subject_id, content, key_topics) VALUES (?, ?, ?, ?)'
        ).run(lesson.id, subjectId, summary.content, JSON.stringify(summary.keyTopics));
        summariesData.push({
          title: lesson.youtube_title || `Aula ${lesson.order_index + 1}`,
          content: summary.content,
        });

        const quizzes = await generateQuizzes(lesson.transcript!, 5);
        const insertQuiz = db.prepare(
          'INSERT INTO quizzes (subject_id, lesson_id, question, options, correct_index, explanation, difficulty) VALUES (?, ?, ?, ?, ?, ?, ?)'
        );
        ...
        const insertRadar = db.prepare(
          'INSERT INTO exam_radar (subject_id, lesson_id, topic, relevance, professor_quote, reasoning) VALUES (?, ?, ?, ?, ?, ?)'
        );
